...
export function renameRoutes(app:FastifyInstance,client:MatcherClient) {
  const schema={body:{type:'object',required:['folder_path'],properties:{folder_path:{type:'string',minLength:1},template:{type:'string'},daily_template:{type:'string'},anime_template:{type:'string'},series_type:{enum:['auto','standard','daily','anime']},release_group:{type:'string'},only_src:{type:'array',items:{type:'string'}}}}};
