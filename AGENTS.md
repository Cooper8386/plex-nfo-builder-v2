...
Create a separate commit for every completed phase after its required checks and build pass, before starting the next phase. This applies even when the user requests multiple phases in one go; never combine those phases into a single commit. Record each phase's own commit SHA in the plan checkpoint.
